...
void CHC::internalFunctionCall(FunctionCall const& _funCall)
{
	...
	if (function)
	{
		...
		// Liraries can have constants as their "state" variables,
...
smt::Expression CHC::summary(FunctionDefinition const& _function)
{
	...
	solAssert(contract, "");
	bool otherContract = contract != m_currentContract && contract->contractKind() == ContractDefinition::ContractKind::Library;
